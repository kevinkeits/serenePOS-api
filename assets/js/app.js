...
document.addEventListener('init', function (event) {
	...
	if (page.id === 'cart') {
		if (getCookie(MSG['cookiePrefix']+'AUTH-TOKEN')=="") doHandlerNotAuthorized();
		else doFetch('external/getCart','_cb=onCompleteFetchCart&_p=cartItemWrapper',false);
	}
...
function onCompleteFetchBanner(data,page) {
	...
	if (data.length > 0) {
		var onclick = '';
		html = '<ons-carousel fullscreen swipeable auto-scroll overscrollable id="car-' + dirPath + '" style="width: 98%; border-radius: 10px; margin: auto; height: ' + (page == "main-banner-slider-wrapper" ? "auto" : 'auto') + '">';
		for (i=0;i<data.length;i++) {
			if (dirPath == 'banner') {
...
function onCompleteFetchCategory(data,page) {
	...
	if (data.length > 0) {
		...
		html += '<li>' +
					'<ons-col style="margin-left: 20px;">' +
						'<img src="#" width="50px" height="50px" style="padding: 10px;">' +
					'</ons-col>' +
				'</li>';
		$('#'+page).html(html);
		$('#'+page).lightSlider({
...
function onCompleteFetchAllProduct(data,page) {
	...
	if (data.length > 0) {
		...
		for (i=0;i<data.length;i++) {
			...
			if (x == 1) html += '<ons-row>';
			html += '<ons-col>' +
						'<ons-card style="padding: 6px; border-radius: 10px; cursor: pointer;" class="item-card" onclick="doOpenPage(\'product-description.html\',{\'ID\':\''+ data[i].ProductID +'\'})">' +
							'<div style="position: relative;">' +
								'<img src="' + urls + '" style="width:100%; height: 160px; border-radius: 6px;">' ;
								if (data[i].Stock == 0) html += '<span id="overlay_text" style="position: relative; left: 50px; top: -60px; font-weight: 500; color: #f1f1f1; width: 100%; background: rgba(0, 0, 0, 0.5); padding: 20px; border-radius: 50px">Habis</span>';
					html += '</div>' +
							'<ons-row>' +
...
function doUpdateStepper(ID,Type) {
	...
	var step = parseInt($('#'+ID).attr("step"));
	var value = parseInt($('#'+ID).val());
	if (Type == "up") {
		if ((value + step) > max) $('#'+ID).val(value);
		else $('#'+ID).val((value + step));
	}
	if (Type == "down") {
		if ((value - step) < min) $('#'+ID).val(value);
		else $('#'+ID).val((value - step));
	}
}
					...
					'<ons-list-item modifier="nodivider">' +
						...
						'<div class="right">' +
							'<span>' +
								...
								'<button onclick="doUpdateStepper(\'qty-'+ data[i].ProductID +'\',\'down\');doCartUpdateQty(\''+ data[i].ProductID +'\')">–</button>' +
								'<input type="number" id="qty-'+ data[i].ProductID +'" value="'+ data[i].Qty +'" min="0" max="1000" step="1" onblur="doCartUpdateQty(\''+ data[i].ProductID +'\')">' +
								'<button onclick="doUpdateStepper(\'qty-'+ data[i].ProductID +'\',\'up\');doCartUpdateQty(\''+ data[i].ProductID +'\')">+</button>' +
							'</span>' +
						'</div>' +
					'</ons-list-item>' +
					'<ons-list-item modifier="nodivider">' +
...
function doHandlerSaveCart(source,isvalid) {
	...
	if(source=="cart") doFetch('external/getCart','_cb=onCompleteFetchCart&_p=cartItemWrapper',false);
}
...
function doHandlerUpdateUser() {
	doFetch('external/getUser','_cb=onCompleteFetchUserDetail&_p=',false);document.querySelector('ons-back-button').click();
}
...
function doHandlerSaveAddress() {
	doFetch('external/getUserAddress','_cb=onCompleteFetchUserAddress&_p=profile-address-wrapper',false);document.querySelector('ons-back-button').click();
}
		...
		for (i=0;i<data.length;i++) {
			if (data[i].IsReply == 1) {
				html += '<div class="box sb2" style="background: rgba(209,213,219)">' +
							'<ons-row><span>'+data[i].Message.replace("'","")+'</span></ons-row>' +
						'</div>';
			} else {
				console.log(i);
				html += '<div class="box sb1" style="background: rgba(37,99,235)">' +
							'<ons-row><span style="color: #fff">'+data[i].Message.replace("'","")+'</span></ons-row>' +
						'</div>';
				if (i == (data.length-1)) {
					html += '<div class="box" style="background: rgb(236, 229, 226); font-size: 8pt">' +
								'<ons-row><span style="color: grey">Mohon menunggu, Kami akan segera membalas pesan anda</span></ons-row>' +
							'</div>';
				}
			}
			...
			html += '<ons-card onclick="doOpenPage(\'transaction-detail.html\',{\'ID\':\''+ data[i].ID +'\'})">' +
					...
					'<ons-row vertical-align="center" style="margin-bottom: 8px;">' +
						...
						'<ons-col style="margin-top: 6px;">' +
							...
							'<span style="color: grey; font-size: 10pt">' + (data[i].PaymentMethodCategory == "gopay" ? "" : data[i].ReferenceID)+ '</span>' +
						'</ons-col>' +
					'</ons-row>' +
					'<ons-row vertical-align="center" style="margin-bottom: 8px;">' +
						'<ons-col>' +
						...
						'</ons-col>' +
					'</ons-row>' +
					(data[i].PaymentMethodCategory == "gopay" ? '<div style="padding-bottom: 24px;"><button onclick="window.location.href='+ data[i].GopayDeepLink +'" style="background-color: #00aed6; color: white; padding: 6px 6px; cursor: pointer; border: none; float: right; width: 40%; border-radius: 10px;">Bayar Pakai Gopay</button></div>' : '') +
				'</ons-card>';
		}
	}
	$('#'+page).html(html);
