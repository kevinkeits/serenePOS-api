...
<button
  id="btnFrmAdd"
  onclick="loadModal('article/form','initQuill()')"
...
<table id="tblarticle" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  ...
  $('#tblarticle').DataTable( {
    ajax: {
      url: apiUrl+'/article/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      { data:'TypeDesc', className:'px-4 py-3 text-sm' },
      { data:'Title', className:'px-4 py-3 text-sm' },
      {
        data:'Status',
        ...
        render: function (data, type, full, meta) {
          var html = '<div class="flex item-center justify-center">' +
                        '<div onclick="showDetailForm(\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          ...
                          '</svg>' +
                        '</div>' +
                        '<div onclick="showDeleteConfirm(\''+full['ID']+'\',\''+full['Title']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">' +
  ...
  function doReloadTable() {
    Pace.restart();
    $('#tblarticle').DataTable().ajax.reload();
    modal.close();
  }

  function doSearchTable() {
    $('#tblarticle').DataTable().search( $('#txtSearch_tblsettings').val() ).draw();
  }

  function showDetailView(ID) {
    loadModal('article/view','onDetailForm(\''+ID+'\')');
  }

  function showDetailForm(ID) {
    loadModal('article/form','onDetailForm(\''+ID+'\')');
  }

  function showDeleteConfirm(ID) {
    Swal.fire({text:'Apakah anda yakin akan menghapus data ini?', icon:'warning', showCancelButton:true, confirmButtonText: 'Ya', cancelButtonText: 'Tidak'})
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('article/doDelete',param);
      }
    });
  }
