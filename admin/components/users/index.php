...
<table id="tblusers" class="w-full whitespace-nowrap">
  <thead>
    <tr class="font-semibold tracking-wide text-left text-gray-500 bg-gray-100 uppercase border-b">
      <th class="px-4 py-3 hidden">Username</th>
      <th class="px-4 py-3">Email</th>
      <th class="px-4 py-3">Nama Lengkap</th>
      <th class="px-4 py-3">Grup Otorisasi</th>
...
<script>
  ...
  $('#tblusers').DataTable( {
    ...
    "ordering": false,
    columns: [
      { data:'UserName', className:'px-4 py-3 text-sm hidden' },
        ...
        render: function (data, type, full, meta) {
          var html = '<div class="flex item-center justify-center">' +
                        '<div onclick="showDetailForm(\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          ...
                          '</svg>' +
                        '</div>' +
                        '<div onclick="showDeleteConfirm(\''+full['ID']+'\',\''+full['FullName']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">' +
