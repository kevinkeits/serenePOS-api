...
<table id="tblstockapproval" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  Pace.restart();
  $('#tblstockapproval').DataTable( {
    ajax: {
      url: apiUrl+'/stockapproval/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      ...
      { data:'Branch', className:'px-4 py-3 text-sm' },
      {
        ...
        render: function (data, type, full, meta) {
          var html = '<span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">Menunggu Persetujuan</span>';
  ...
  function doReloadTable() {
    Pace.restart();
    $('#tblstockapproval').DataTable().ajax.reload();
    modal.close();
  }

  function doSearchTable() {
    $('#tblstockapproval').DataTable().search( $('#txtSearch_tblstockapproval').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('stockapproval/form','onDetailForm(\''+ID+'\')');
  }
</script>