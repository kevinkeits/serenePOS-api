...
<button
  onclick="loadModal('stockrequest/form','onAddForm()')"
...
<table id="tblstockrequest" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  ...
  $('#tblstockrequest').DataTable( {
    ajax: {
      url: apiUrl+'/stockrequest/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      ...
      { data:'Branch', className:'px-4 py-3 text-sm' },
      {
        ...
        render: function (data, type, full, meta) {
          var html = '<span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">Draft</span>';
          if (full['RequestStatus'] == '2') html = '<span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">Menunggu Persetujuan</span>';
          if (full['RequestStatus'] == '3') html = '<span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">Disetujui</span>';
          if (full['RequestStatus'] == '4') html = '<span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">Dikirim</span>';
        ...
        render: function (data, type, full, meta) {
          var html = '<div class="flex item-center justify-center">' +
                        '<div onclick="showDetailForm(\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          ...
                          '</svg>' +
                        '</div>';
                        if (full['RequestStatus'] == '1') {
                          html += '<div onclick="showDeleteConfirm(\''+full['ID']+'\',\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                                    '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">' +
  ...
  function doReloadTable() {
    Pace.restart();
    $('#tblstockrequest').DataTable().ajax.reload();
    modal.close();
  }

  function doSearchTable() {
    $('#tblstockrequest').DataTable().search( $('#txtSearch_tblstockrequest').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('stockrequest/form','onDetailForm(\''+ID+'\')');
  }
  ...
  function showDeleteConfirm(ID,Label) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('stockrequest/doDelete',param);
      }
    });
  }
</script>