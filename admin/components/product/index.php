...
<button
  onclick="loadModal('product/form','fetchBranch();fetchCategory();')"
...
<script>
  ...
  $('#tblusers').DataTable( {
    ajax: {
      url: apiUrl+'/product/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      { data:'Code', className:'px-4 py-3 text-sm' },
      { data:'Name', className:'px-4 py-3 text-sm' },
      { data:'BranchName', className:'px-4 py-3 text-sm' },
      { data:'Stock', className:'px-4 py-3 text-sm' },
      {
        data:'Status',
  ...
  function showDetailForm(ID) {
    loadModal('product/form','onDetailForm(\''+ID+'\')');
  }
  ...
  function showDeleteConfirm(ID,Label) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('product/doDelete',param);
      }
    });
  }
</script>