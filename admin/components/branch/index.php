...
<button
  onclick="loadModal('branch/form','fetchState();fetchAllUser();')"
...
<script>
  ...
  $('#tblusers').DataTable( {
    ajax: {
      url: apiUrl+'/branch/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      { data:'Name', className:'px-4 py-3 text-sm' },
      { data:'DistrictName', className:'px-4 py-3 text-sm' },
      { data:'CityName', className:'px-4 py-3 text-sm' },
      { data:'StateName', className:'px-4 py-3 text-sm' },
      {
        data:'Status',
  ...
  function showDetailForm(ID) {
    loadModal('branch/form','onDetailForm(\''+ID+'\')');
  }
  ...
  function showDeleteConfirm(ID,Label) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('branch/doDelete',param);
      }
    });
  }
</script>