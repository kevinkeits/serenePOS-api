...
<button
  onclick="loadModal('customer/form','fetchState();')"
  class="hidden w-32 float-right px-4 py-2 text-sm text-white bg-red-600 border border-transparent rounded-lg active:bg-red-800 hover:bg-red-700 focus:outline-none focus:shadow-outline-red"
>
  Tambah Baru
</button>
...
<table id="tblcustomer" class="w-full whitespace-nowrap">
  <thead>
    <tr class="font-semibold tracking-wide text-left text-gray-500 bg-gray-100 uppercase border-b">
      <th class="px-4 py-3 hidden">Code</th>
...
<script>
  ...
  $('#tblcustomer').DataTable( {
    ...
    "ordering": false,
    columns: [
      { data:'Code', className:'px-4 py-3 text-sm hidden' },
      { data:'Name', className:'px-4 py-3 text-sm' },
      { data:'Phone', className:'px-4 py-3 text-sm' },
      { data:'Email', className:'px-4 py-3 text-sm' },
      {
        data:'Status',
