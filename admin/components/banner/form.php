<form id="frmBanner" onsubmit="return doSubmitForm(event,'banner/doSave','frmBanner')" enctype="multipart/form-data">
  	<div class="mt-4 mb-6">
    	<div id="lblMdlTitle" class="mb-2 text-lg font-semibold text-gray-700">Tambah Banner Baru</div>
		<div class="text-sm text-gray-700">
			...
			<label class="block mt-4">
				<span>Produk Keyword</span>
				<input
					id="txtFrmProductKeyword"
					name="txtFrmProductKeyword"
					type="text"
					maxlength="255"
					class="border p-2 rounded w-full mt-1 text-sm form-input focus:border-gray-400 focus:outline-none focus:shadow-outline-gray"
					placeholder="Ketik disini"
...
<script>
	function onDetailForm(ID) {
		$('#lblMdlTitle').html('Ubah Banner');
		$('#hdnFrmID').val(ID);
		$('#hdnFrmAction').val('edit');
		doFetch('banner/get','_i='+ID);
	}
	function onCompleteFetch(data) {
		Swal.close();
		$('#txtFrmName').val(data.Name);
		$('#txtFrmProductKeyword').val(data.Keyword);
		$('#txtFrmURL').val(data.URL);
		if (data.ImagePath != null && data.ImagePath != '') {
			$('#imgFile').attr('src',uploadedUrl + '/banner/' + data.ImagePath);
			$('#imgModBtn').show();
		} else {
			$('#hdnAttached').val("");
			$('imgModBtn').hide();
	...
	function execUpload(ID) {
		doUpload('banner/doUpload','inpFile',ID);
	}

	function removePhoto(ID) {
		var param = '_i='+ID;
		doSubmit('banner/doDeletePic',param);
	}
</script>