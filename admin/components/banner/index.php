...
<table id="tblbanner" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  ...
  $('#tblbanner').DataTable( {
    ajax: {
      url: apiUrl+'/banner/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
  ...
  function doSearchTable() {
    $('#tblbanner').DataTable().search( $('#txtSearch_tblbanner').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('banner/form','onDetailForm(\''+ID+'\')');
  }
  ...
  function showDeleteConfirm(ID,Label) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('banner/doDelete',param);
      }
    });
  }
