...
<button
  onclick="loadModal('discount/form','fetchBranch()')"
...
<table id="tbldiscount" class="w-full whitespace-nowrap">
  <thead>
    <tr class="font-semibold tracking-wide text-left text-gray-500 bg-gray-100 uppercase border-b">
      ...
      <th class="px-4 py-3">Sampai Tanggal</th>
...
<script>
  ...
  $('#tbldiscount').DataTable( {
    ajax: {
      url: apiUrl+'/discount/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      { data:'ProductCode', className:'px-4 py-3 text-sm' },
      { data:'ProductName', className:'px-4 py-3 text-sm' },
      { data:'StartDate', className:'px-4 py-3 text-sm' },
      { data:'EndDate', className:'px-4 py-3 text-sm' },
      { data:'strDiscount', className:'px-4 py-3 text-sm' },
      {
        data:'Status',
        ...
        render: function (data, type, full, meta) {
          var html = '<div class="flex item-center justify-center">' +
                        '<div onclick="showDetailForm(\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          ...
                          '</svg>' +
                        '</div>' +
                        '<div onclick="showDeleteConfirm(\''+full['ID']+'\',\''+full['ProductName']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">' +
  ...
  function doReloadTable() {
    Pace.restart();
    $('#tbldiscount').DataTable().ajax.reload();
    modal.close();
  }

  function doSearchTable() {
    $('#tbldiscount').DataTable().search( $('#txtSearch_tbldiscount').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('discount/form','onDetailForm(\''+ID+'\')');
  }
  ...
  function showDeleteConfirm(ID,Label) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('discount/doDelete',param);
      }
    });
  }
</script>