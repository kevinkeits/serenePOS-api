...
<button
  id="btnFrmAdd"
  onclick="loadModal('category/form')"
...
<table id="tblcategory" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  ...
  $('#tblcategory').DataTable( {
    ajax: {
      url: apiUrl+'/category/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
  ...
  function doSearchTable() {
    $('#tblcategory').DataTable().search( $('#txtcategory').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('category/form','onDetailForm(\''+ID+'\')');
  }
  ...
  function showDeleteConfirm(ID,Label) {
    ...
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('category/doDelete',param);
      }
    });
  }
