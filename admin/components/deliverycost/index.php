...
<button
  onclick="loadModal('deliverycost/form','fetchStateFrom();fetchStateTo();')"
...
<table id="tbldeliverycost" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  Pace.restart();
  $('#tbldeliverycost').DataTable( {
    ajax: {
      url: apiUrl+'/deliverycost/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      { data:'DistrictFrom', className:'px-4 py-3 text-sm' },
      { data:'DistrictTo', className:'px-4 py-3 text-sm' },
      {
        data:'Fee',
        className:'px-4 py-3 text-sm',
        render: function (data, type, full, meta) {
          return doFormatNumber(data)
        },
      },
      {
        data:'Status',
        className:'px-4 py-3 text-sm',
        render: function (data, type, full, meta) {
          var html = data==1 ? '<span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">Aktif</span>' : '<span class="px-2 py-1 font-semibold leading-tight text-white bg-red-400 rounded-full">Tidak Aktif</span>';
          return html
        },
      },
      {
        ...
        render: function (data, type, full, meta) {
          var html = '<div class="flex item-center justify-center">' +
                        '<div onclick="showDetailForm(\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          ...
                          '</svg>' +
                        '</div>' +
                        '<div onclick="showDeleteConfirm(\''+full['ID']+'\',\''+full['Name']+'\')" class="w-4 mr-2 transform hover:text-red-500 hover:scale-110 cursor-pointer">' +
                          '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">' +
  ...
  function doReloadTable() {
    Pace.restart();
    $('#tbldeliverycost').DataTable().ajax.reload();
    modal.close();
  }

  function doSearchTable() {
    $('#tbldeliverycost').DataTable().search( $('#txtSearch_tbldeliverycost').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('deliverycost/form','onDetailForm(\''+ID+'\')');
  }

  function showDeleteConfirm(ID,Label) {
    Swal.fire({html:'Apakah anda yakin akan menghapus data ini?', icon:'warning', showCancelButton:true, confirmButtonText: 'Ya', cancelButtonText: 'Tidak'})
    .then((result) => {
      if (result.isConfirmed) {
        var param = '_i='+ID;
        doSubmit('deliverycost/doDelete',param);
      }
    });
  }
</script>